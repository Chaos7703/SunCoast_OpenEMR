...
//		else
  ...
  protected function fetchProblemsSubinfo(&$problemArray, $idFieldName) {
    ...
    $servicePlan = $this->sqlFetchArray(
			$this->sqlStatements['NEW']['TCMServicePlanMostRecentFinalized'],
			array(
    ...
    if(empty($servicePlan)) {  // no finalized plan, use most recent unfinalized
      $servicePlan = $this->sqlFetchArray(
        $this->sqlStatements['NEW']['TCMServicePlanMostRecent'],
        array(
          'patientId' => $this->patientId,
        )
      );
    ...
    // Create prototype Note from Service Plan info
		...
		$this->formData['Note']['ClientName'] = $servicePlan['ClientName'];
    $this->formData['Note']['DateWritten'] = $servicePlan['ReportDate'];
