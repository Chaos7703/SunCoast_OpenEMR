...
function ActivityRecord() {
  ...
  this.ActivityTypeId = ko.observable();
...
//console.log( 'ProblemRecord.GoalsAdd: data = ' + ko.toJSON(data,null,4));
		...
		goal.Id(data.Id);
		goal.Goal(data.Goal);
//console.log( 'GoalsAdd: data.Objectives (observable ' + ko.isObservable(data.Objectives) + ') = ' + ko.toJSON(data.Objectives,null,4));    
		data.Objectives.forEach(function(objective) {
//alert( 'ProblemRecord.GoalsAdd: objective = ' + ko.toJSON(objective,null,4));    
...
function ServicePlanProblemRecord() {
  ...
  this.Goals = ko.observableArray();
	this.GoalsAdd = function(data) {
		...
		goal.Goal(data.Goal);
		data.Objectives.forEach(function(objective) {
// alert('SPPR objective = ' + ko.toJSON(objective,null,4));
			goal.ObjectivesAdd(objective);
		});
//alert( 'SPPR goal = ' + ko.toJSON(goal,null,4));       
		self.Goals.push(goal);
	};
	this.GoalsAddNew = function() {
		self.Goals.push(new GoalRecord);
